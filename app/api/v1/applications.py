...
# ----------------------------------------------------------
@router.get(
    "/my", response_model=List[JobApplicationResponse], status_code=status.HTTP_200_OK
)
...
async def get_application_by_job(
    ...
    """
    if current_user.role.value != UserRole.APPLICANT:
        raise HTTPException(
            status_code=401, detail="Not authorized to perform this action"
        )

    application = await app_service.get_application_by_applicant_and_job(
