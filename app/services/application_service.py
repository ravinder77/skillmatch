...
class ApplicationService:
    def __init__(
        self, application_repo: ApplicationRepository, job_repository: JobRepository
    ):
        self.application_repository = application_repo
        self.job_repository = job_repository

    async def apply_to_job(
        self, applicant_id: int, job_id: int, resume_file: Optional = None
    ) -> Application:
        """
        ...
        if not job.is_active:
            raise HTTPException(
                status_code=400, detail="Not accepting application currently"
            )

        # Check if already applied for the job
        existing_application = (
            await self.application_repository.get_by_applicant_and_job(
                job_id, applicant_id
            )
        )
    ...
    # ----------------------------------------------------------
    async def get_all_applications_by_applicant(
        self,
        applicant_id: int,
    ) -> List[Application]:
        # Fetch all applications by this user
        applications = await self.application_repository.get_all_by_applicant(
            applicant_id
        )
    ...
    # ----------------------------------------------------------
    async def get_application_by_applicant_and_job(
        self, applicant_id: int, job_id: int
    ) -> Application:
        """Fetch a job application by job ID and candidate's ID."""
        application = await self.application_repository.get_by_applicant_and_job(
            job_id, applicant_id
        )
